...
export const GET: RequestHandler = async ({ url, params }) => {
	const type = z.literal(["xml", "json"]).safeParse(params.type);
	if (type.error)
		return new Response(`Invalid type.\nShould be: feed.{xml, json}`, {
	...
	const feed = new Feed({
		...
		link: `${url.protocol}//${url.host}`,
		language: "ja",
		updated: lastUpdateDate,
		image: `${url.protocol}//${url.host}/kirura/768p.png`,
		author: {
			name: "きるら",
	...
	if (type.data === "json") return new Response(feed.json1());
