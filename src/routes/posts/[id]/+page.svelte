...
<main
	class={cx(
		container(),
		css({ py: "8", animationName: "slide-from-top, fade-in" }),
	)}
>
	<article>
		<div
			data-has-image={data.coverImage || undefined}
			class={css({
				pos: "relative",
				borderBottomWidth: "1px",
				"&[data-has-image]": { borderWidth: "1px" },
			})}
		>
			{#if data.coverImage}
				<div
			...
			>
				...
				{#if data.subtitle}
					<p class={css({ maxW: "prose", fontStyle: "italic" })}>
						{data.subtitle}
					</p>
				{/if}
			</hgroup>
